...
/* creating the BoundingBox table */
    ret = sqlite3_exec (db_handle, "CREATE TABLE BBox_Test (pk_uid INTEGER, "
        ...
        "name TEXT, count INTEGER, measure DOUBLE, odd BLOB)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
...
/* inserting few rows into the BoundingBox table */
    ret = sqlite3_exec (db_handle, "INSERT INTO BBox_Test "
        ...
        "VALUES (1, 1, 2, 3, 4, 4326, 'alpha', 10, 10.1, zeroblob(4))", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    ret = sqlite3_exec (db_handle, "INSERT INTO BBox_Test "
        "(pk_uid, min_x, min_y, max_x, max_y, srid, name, count, measure, odd) "
        "VALUES (2, 2, 3, 4, 5, 4326, 'beta', 20, 20.2, zeroblob(4))", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    ret = sqlite3_exec (db_handle, "INSERT INTO BBox_Test "
        "(pk_uid, min_x, min_y, max_x, max_y, srid, name, count, measure, odd) "
        "VALUES (3, 3, 4, 5, 6, 3003, 'gamma', 30, 30.3, NULL)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    ret = sqlite3_exec (db_handle, "INSERT INTO BBox_Test "
        "(pk_uid, min_x, min_y, max_x, max_y, srid, name, count, measure, odd) "
        "VALUES (4, 1, 2, NULL, 4, 4326, 'delta', 40, NULL, NULL)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    ret = sqlite3_exec (db_handle, "INSERT INTO BBox_Test "
        "(pk_uid, min_x, min_y, max_x, max_y, srid, name, count, measure, odd) "
        "VALUES (5, 1, 2, 3, 4, NULL, 'epsilon', NULL, NULL, NULL)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
...
/* creating the VirtualBBox table #1 */
    ret = sqlite3_exec (db_handle, "CREATE VIRTUAL TABLE test1 USING "
        ...
        "srid, 0, pk_uid, name, count, measure, odd)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
...
/* creating the VirtualBBox table #2 */
    ret = sqlite3_exec (db_handle, "CREATE VIRTUAL TABLE test2 USING "
        "VirtualBBox('BBox_Test', 'min_x', min_y, \"max_x\", 'max_y', 3003, 0)", NULL, NULL, &err_msg);
    if (ret != SQLITE_OK) {
...
/* creating the VirtualBBox table #3 - failure */
    ret = sqlite3_exec (db_handle, "CREATE VIRTUAL TABLE test3 USING "
        "VirtualBBox('BBox_Test', 'min_x', min_y, \"max_x\", 'max_y', 3003)", NULL, NULL, &err_msg);
    if (ret == SQLITE_OK) {
...
/* creating the VirtualBBox table #4 - failure */
    ret = sqlite3_exec (db_handle, "CREATE VIRTUAL TABLE test3 USING "
        "VirtualBBox('BBox_Test', 'min_x', min_y, \"max_x\", 'wrong', 3003, 0)", NULL, NULL, &err_msg);
    if (ret == SQLITE_OK) {
...
/* creating the VirtualBBox table #46 - failure */
    ret = sqlite3_exec (db_handle, "CREATE VIRTUAL TABLE test3 USING "
        "VirtualBBox('wrong', 'min_x', min_y, \"max_x\", 'max_y', 3003, 0)", NULL, NULL, &err_msg);
    if (ret == SQLITE_OK) {
