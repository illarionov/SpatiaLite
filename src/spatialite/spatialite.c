...
/* creating the VIRTS_GEOMETRY_COLUMNS_TIME table */
    strcpy (sql, "CREATE TABLE IF NOT EXISTS ");
...
/* creating an INDEX corresponding to the SRID FK */
    ...
    if (ret != SQLITE_OK)
	goto error;
/* creating the GEOMETRY_COLUMN triggers */
    ...
    strcat (sql, "FOR EACH ROW BEGIN\n");
    strcat (sql,
	    "SELECT RAISE(ABORT,'coord_dimension must be one of 1,2,3,4')\n");
    strcat (sql, "WHERE NOT(NEW.coord_dimension IN (1,2,3,4));\n");
    strcat (sql, "END");
    ret = sqlite3_exec (sqlite, sql, NULL, NULL, &errMsg);
    if (ret != SQLITE_OK)
	goto error;
...
static int
get_real_names (sqlite3 * sqlite, const char *table, const char *column,
...
static void
updateGeometryTriggers (sqlite3 * sqlite, const char *table, const char *column)
{
/* updates triggers for some Spatial Column */
    ...
    if (!get_real_names (sqlite, table, column, &p_table, &p_column))
      {
	  spatialite_e
...
/* updating metadata tables */
    ...
    if (checkSpatialMetaData (sqlite) == 3)
	  ...
	  strcpy (sql,
		  "INSERT OR REPLACE INTO virts_geometry_columns_auth (virt_name, virt_geometry, ");
	  strcat (sql, "hidden) VALUES (");
	  strcat (sql, "'");
	  strcat (sql, xtable);
	  strcat (sql, "', 'Geometry', 0)");
	  ret = sqlite3_exec (sqlite, sql, NULL, NULL, &errMsg);
	  if (ret != SQLITE_OK)
	    {
		spatialite_e ("RegisterVirtualGeometry() error: \"%s\"\n",
	  ...
	  /* inserting a row into GEOMETRY_COLUMNS_STATISTICS */
	  strcpy (sql,
		  "INSERT OR REPLACE INTO virts_geometry_columns_statistics (virt_name, virt_geometry) ");
	  strcat (sql, "VALUES (");
	  strcat (sql, "'");
	  strcat (sql, xtable);
	  strcat (sql, "', 'Geometry')");
