		   ...
		   for (i_col = 0; i_col < sqlite3_column_count (stmt); i_col++)
		     {
			  ...
			  else if (sqlite3_column_type (stmt, i_col) !=
				   SQLITE_BLOB)
			       ok = 0;
			  else
			    {
     ...
     if (metadata_version == 3)
       {
	    ...
	    strcat (sql,
		    "read_only, hidden) VALUES (Lower(?), Lower(?), 0, 0)");
	    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
	    if (ret != SQLITE_OK)
	      {
		   spatialite_e ("AddGeometryColumn: \"%s\"\n",
     ...
     else
       {
	    ...
	    sql_statement =
		 ...
		 ("INSERT OR REPLACE INTO virts_geometry_columns "
		  ...
		  "VALUES (Lower(%Q), 'geometry', %Q, %d)", table, xgtype,
		  srid);
       }
     ret = sqlite3_exec (sqlite, sql_statement, NULL, NULL, &errMsg);
     sqlite3_free (sql_statement);
		   ...
		   if (to_be_fixed)
		     {
			  ...
			  status =
			       recover_spatial_index (sqlite, table, column);
			  if (status < 0)
			    {
				 /* some unexpected error occurred */
				 goto fatal_error;
	    ...
	    else
	      {
		   ...
		   switch (pt->DimensionModel)
		     ...
		     case GAIA_XY_Z_M:
			  ...
			  break;
		     case GAIA_XY_Z:
			  gaiaPrependPointZToDynamicLine (dyn, pt->X, pt->Y,
							  pt->Z);
			  break;
		     case GAIA_XY_M:
			  gaiaPrependPointMToDynamicLine (dyn, pt->X, pt->Y,
							  pt->M);
			  break;
		     default:
		   ...
		   if (vertex >= 0 && vertex < line->Points)
		     {
			  if (line->DimensionModel == GAIA_XY_Z)
			    {
				 gaiaGetPointXYZ (line->Coords, vertex, &x, &y,
						  &z);
			    ...
			    {
				 gaiaGetPointXYM (line->Coords, vertex, &x, &y,
						  &m);
			  ...
			  else if (line->DimensionModel == GAIA_XY_Z_M)
			    {
				 ...
				 result->Srid = geo->Srid;
				 gaiaAddPointToGeomCollXYZM (result, x, y, z,
							     m);
			    }
			  else
			    {
				 ...
				 if (ring->DimensionModel == GAIA_XY_Z)
				   {
					gaiaGetPointXYZ (ring->Coords, iv, &x,
							 &y, &z);
					gaiaSetPointXYZ (line->Coords, iv, x, y,
							 z);
				   }
				 else if (ring->DimensionModel == GAIA_XY_M)
				   {
					gaiaGetPointXYM (ring->Coords, iv, &x,
							 &y, &m);
					gaiaSetPointXYM (line->Coords, iv, x, y,
							 m);
				   }
				 else if (ring->DimensionModel == GAIA_XY_Z_M)
				   {
     ...
     if (strlen (text) == 5)
       {
	    ...
	    char letterLSD = '\0';
	    numMatch =
		 sscanf (text, "%u%c%c", &digit100, &letterMSD, &letterLSD);
	    if (numMatch != 3)
	      {
		   sqlite3_result_null (context);
		   return;
		   ...
		   if (cnt == entity)
		     {
			  ...
			  if (point->DimensionModel == GAIA_XY_Z)
			       ...
			       gaiaAddPointToGeomColl (result, point->X,
						       point->Y);
			  goto skip;
		     }
		   point = point->Next;
				 ...
				 if (line->DimensionModel == GAIA_XY_Z)
				   {
					gaiaGetPointXYZ (line->Coords, iv, &x,
							 &y, &z);
					gaiaSetPointXYZ (line2->Coords, iv, x,
							 y, z);
				   }
				 else if (line->DimensionModel == GAIA_XY_M)
				   {
					gaiaGetPointXYM (line->Coords, iv, &x,
							 &y, &m);
					gaiaSetPointXYM (line2->Coords, iv, x,
							 y, m);
				   }
				 else if (line->DimensionModel == GAIA_XY_Z_M)
				   {
			  ...
			  for (ib = 0; ib < polyg2->NumInteriors; ib++)
			    {
				 ...
				 for (iv = 0; iv < ring_out->Points; iv++)
				   {
					if (ring_in->DimensionModel ==
					    GAIA_XY_Z)
					  {
					       gaiaGetPointXYZ (ring_in->Coords,
								iv, &x, &y, &z);
					       gaiaSetPointXYZ (ring_out->
								Coords, iv, x,
								y, z);
					  }
					else if (ring_in->DimensionModel ==
						 GAIA_XY_M)
					  {
						 ...
						 GAIA_XY_Z_M)
					  {
					       gaiaGetPointXYZM (ring_in->
								 Coords, iv, &x,
								 &y, &z, &m);
					       gaiaSetPointXYZM (ring_out->
								 Coords, iv, x,
								 y, z, m);
					  }
					else
					...
					if (is_perimeter)
					  {
					       ...
					       while (polyg)
						 {
						      /* exterior Ring */
						      ring = polyg->Exterior;
						      l = gaiaGeodesicTotalLength (a, b, rf, ring->DimensionModel, ring->Coords, ring->Points);
						      if (l < 0.0)
							{
							     length = -1.0;
							     break;
	    ...
	    if (use_ellipsoid >= 0)
	      {
		   ...
		   /* attempting to identify the corresponding ellipsoid */
		   if (getEllipsoidParams (sqlite, geo->Srid, &a, &b, &rf))
			ret = gaiaGeodesicArea (geo, a, b, use_ellipsoid,
						&area);
		   else
			ret = 0;
#else
		   ...
		   if (pt0 == 1 && pt1 == 1 && ln0 == 0 && ln1 == 0 && pg0 == 0
		       && pg1 == 0)
		     {
			  ...
			  if (use_spheroid)
			    {
				 dist =
				      gaiaGeodesicDistance (a, b, rf, y0, x0,
							    y1, x1);
				 if (dist <= ref_dist)
				      sqlite3_result_int (context, 1);
				 else
...
register_spatialite_sql_functions (void *p_db, void *p_cache)
{
     ...
     sqlite3_create_function (db, "GeomFromEWKB", 1, SQLITE_ANY, 0,
			      fnct_FromEWKB, 0, 0);
     sqlite3_create_function (db, "AsEWKB", 1, SQLITE_ANY, 0, fnct_ToEWKB, 0,
			      0);
     sqlite3_create_function (db, "AsEWKT", 1, SQLITE_ANY, 0, fnct_ToEWKT, 0,
			      0);
     ...
     sqlite3_create_function (db, "ST_nrings", 1, SQLITE_ANY, 0, fnct_NRings, 0,
			      0);
     sqlite3_create_function (db, "ToGARS", 1, SQLITE_ANY, 0, fnct_ToGARS, 0,
			      0);
